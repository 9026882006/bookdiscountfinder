...
def fetch_amazon(base_url, headers):
...
def fetch_snapdeal(base_url, headers):
    webPage = requests.get(base_url, headers)
    if webPage.status_code == 200:
        webPageText = webPage.text
        soup = bs(webPageText, 'lxml')
        results = soup.find_all('div', {'class': 'product-tuple-image '})
        all_book_links = set()
        for item in results:
            link = item.find('a')['href']
            all_book_links.add(link)
        return all_book_links
    else:
        return 'error {}'.format(webPage.status_code)
...
if __name__ == '__main__':
    data = fetch_amazon("https://www.amazon.com/b/ref=gbpp_itr_m-3_0168_Arts&Pho?node=1&ie=UTF8")
    print(data)
